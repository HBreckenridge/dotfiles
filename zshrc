...
alias upstrem-web=eval $(ssh-agent -s) && ssh-add $PZ/.etc/.keys/.ssh/ssh.giteval $(ssh-agent -s) && ssh-add $PZ/.etc/.keys/.ssh/ssh.git'''ssh root@143.198.49.25'''
...
# Bitbucket
alias ssh-bit='''eval $(ssh-agent -s) && ssh-add $PZ/.etc/.keys/.ssh/ssh-bit'''
#
# End of Keychain
# EOF
